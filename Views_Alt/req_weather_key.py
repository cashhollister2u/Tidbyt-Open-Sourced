...
class Weather_Api_key:
    ...
    def run(self):
        ...
        self.user.displayLoaded = 'req_weather_api_key'
        while True:
            self.main_canvas.Clear()

            # close view gracefully
            if self.user.channel != 'req_weather_api_key':
                print('Weather api required loop terminated')
                return self.user.channel

            len_text = graphics.DrawText(self.main_canvas, font, position, 16, white, "Add openweathermap.org api key to Secrets/api_keys.py")
